...
> link-color와 link-underline-color를 변경시켜주면 모든 hyperlink가 들어가는 코드는 색상이 변경된다.
{: .prompt-danger }

그러므로 이것은 잠시 보류를 하도록 하고, 나중에 따로 내가 포스트에 들어가는 hyperlink의 색상을 변경하는 방법을 찾으면 포스팅을 다시 추가하겠다.
